% This script performs all 4DN Lab data analysis on Hi-C and RNA-seq
% Scott Ronquist, 4/6/18
...
% get main path
if isunix
    addpath(genpath('/Volumes/internal_4DN/tools/matlab'))
    selpath.main = '/Volumes/internal_4DN';
else
    addpath(genpath('//172.17.109.24/internal_4DN/tools/matlab'))
    selpath.main = '//172.17.109.24/internal_4DN';
end

% get subpaths
switch data_set
    case 'test'
        load('sf_pipeline_testdata.mat')
        addpath(genpath(selpath.rnaseq))
        addpath(genpath(selpath.hic))
    otherwise
        % RNA-seq data folder
        selpath.rnaseq = uigetdir([selpath.main,'/projects/'],'Select RNA-seq data folder');
        addpath(genpath(selpath.rnaseq))
        
        % Hi-C data folder
        selpath.hic = uigetdir([selpath.main,'/projects/'],'Select Hi-C data folder');
        addpath(genpath(selpath.hic))
        
        % output folder
        selpath.out = uigetdir([selpath.main,'/projects/'],'Select output folder');
end

% get sample names
if isunix
    listing=rdir([selpath.hic,'/**/*.hic'],'',selpath.hic);
else
    listing=rdir([selpath.hic,'\**\*.hic'],'',selpath.hic);
end
samples.hic = cell(length(listing),1);
count = 1;
for i = 1:length(listing)
    slash_loc = sort([strfind(listing(i).name,'\'),...
        strfind(listing(i).name,'/')]);
    if length(slash_loc)<2
        samples.hic{i} = sprintf('unknown%d',count);
        count = count+1;
    else
        samples.hic{i} = listing(i).name(slash_loc(end-1)+1:slash_loc(end)-1);
    end
end

%% Format Data
% this section formats the .txt files from Hi-C and RNA-seq pipeline and
% converts them to MATLAB files. all Hi-C data and information is contained
% within the "R" structure
