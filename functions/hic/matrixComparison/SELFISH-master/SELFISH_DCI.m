...
%   'INTERVAL'         - The interval in bp for which DCIs are detected.
    ...
    avgDiag1(a+1) = accumarray(c,H1(:,3),[],@(x) nanmean(x,1));
    avgSTD1(a+1)  = accumarray(c,H1(:,3),[],@(x) nanstd(x,1));

    [a,~,c]  = unique(interaction_dist2);
    avgDiag2 = zeros(max(interaction_dist2)+1,1);
    avgSTD2 = zeros(max(interaction_dist2)+1,1);
    avgDiag2(a+1) = accumarray(c,H2(:,3),[],@(x) nanmean(x,1));
    avgSTD2(a+1)  = accumarray(c,H2(:,3),[],@(x) nanstd(x,1));
