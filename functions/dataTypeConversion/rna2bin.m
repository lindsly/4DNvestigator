function [rna_bin,rna_gene_names_bin]= rna2bin(rnaseq,geneName,geneLocs,binSize,chrBinLengths,method)
...
for chr=1:length(chrBinLengths)
    ...
    chr_idx = geneLocs(:,1)==chr;
    rna_bin{chr,1} = zeros(chrBinLengths(chr),size(rnaseq,2));
    
    chr_rnaseq = rnaseq(chr_idx,:);
    chr_gene_name = geneName(chr_idx,:);
    chr_gene_locs = geneLocs(chr_idx,:);
    ...
    max_length_ref = max([diff(chr_gene_locs(:,2:3),1,2),ones(size(chr_gene_locs,1),1)*binSize],[],2);
    gene_overlap_percentages = (overlapRatio./repmat(max_length_ref',size(overlapRatio,1),1))*binSize;
