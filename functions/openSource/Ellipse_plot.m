function h = Ellipse_plot(A, C, plot_col, plot_style, varargin)
%
...
%----------------------------------------
if strcmp(Type,'2D'),
    h = plot(X(1,:),X(2,:),'Color',plot_col,'LineWidth',1.3,'LineStyle',plot_style);
%     hold on;
%     plot(C(1),C(2),'r*');
%     axis equal, grid
%     axis equal
...
%     h = surfl(XX,YY,ZZ); 
    color_ = repmat(ones(size(ZZ)),[1,1,3]);
    for sr = 1:size(color_,3)
        color_(:,:,sr) = color_(:,:,sr)*plot_col(sr);
    end
    h = surf(XX,YY,ZZ,color_); 
    set(h, 'FaceAlpha', 0.1)
    shading interp
